...
<tr id="{{note.pk}}">
  <td>
    {% if note.from_user.nickname %} {{note.from_user.nickname}} {% else %}
    {{note.from_user.username}} {% endif %}
  </td>
  <td>
    <a
      class="custom-link"
...
<li class="nav-item dropdown d-flex align-items-center">
  {% if request.user.tree_notice or request.user.note_notice %} {% if
  request.user.notice_tree and request.user.notice_note %}
  <a
    ...
    aria-expanded="false"
  >
    <img src="{% static "images/종.png" %}" alt="" style="width: 20px;
    height:20px;">
  </a>
  <ul
  ...
  <a
    ...
    onclick="apen(event)"
  >
    <img class="bell" src="{% static "images/종2.png" %}" alt="" style="width:
    20px; height:20px;">
  </a>
  {% csrf_token %}
  <ul
  ...
  axios({
    ...
    if (items.length > 0) {
      for (let i = 0; i < items.length; i++) {
        let time = moment(items[i][0])
          .add(9, "hours")
          .format("YYYY년 MM월 D일, a h:mm:ss");
        if (items[i][1][2] === "card") {
          ...
          h6.innerText = time;
          p.innerText = `${items[i][1][1]} 님이 벽난로 ${
            items[i][1][0]
          } 을 남겼어요.`;
          p.className = "mb-0";
          a.appendChild(h6);
